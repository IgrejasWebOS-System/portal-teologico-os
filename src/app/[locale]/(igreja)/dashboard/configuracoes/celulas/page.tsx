...
export default async function CelulasPage() {
  ...
  if (error) {
    console.error("[celulas/page]", error);
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <PageHeader
        icon={GitBranch}
        title="Células"
        description="Grupos vinculados a uma igreja ou sub-congregação"
      ...
      {error && (
        <div className="px-4 py-3 rounded-lg bg-iw-error-bg border border-iw-error text-iw-error text-sm font-medium">
          Erro ao carregar as células: {error.message}
        </div>
      )}
      ...
      <CongregacoesListClient
        rows={rows}
        setores={setores}
        editBasePath="/dashboard/configuracoes/celulas"
        emptyIcon={<GitBranch className="w-10 h-10 text-iw-muted/30 mx-auto mb-3" />}
        ...
        novoHref="/dashboard/configuracoes/celulas/nova"
        novoLabel="Nova Célula"
      />
    </div>
  );
}
