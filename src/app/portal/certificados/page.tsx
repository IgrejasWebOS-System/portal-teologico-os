...
export default async function MeusCertificadosPage() {
  ...
  const { data: certificados } = await supabase
    .from("certificates")
    .select("id, numero_certificado, nome_curso, carga_horaria, emitido_em")
    .eq("user_id", user.id)
        ...
        {(!certificados || certificados.length === 0) ? (
          ...
          <div className="flex flex-col gap-4">
            {certificados.map((c) => (
              <div
                key={c.id}
                className="bg-iw-surface border border-iw-border rounded-2xl p-5 shadow-sm flex items-center justify-between gap-4"
              >
                <div>
