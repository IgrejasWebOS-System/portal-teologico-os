...
| ... | (094 a 108: ver `list_migrations` do MCP do Supabase — regionais, ministérios, professores login vinculado, mutirão de cadastro professor/aluno, etc.) |
| `109_forma_pagamento_debito_credito.sql` | Separa `CARTAO` (genérico) em `DEBITO`/`CREDITO` no constraint de `fin_contas_receber.forma_pagamento_prevista` — pedido do Joaquim (20/09/2026), pra tela de conferência de mensalidades do mutirão de cadastro do aluno. Só `fin_contas_receber`; `fin_contas_a_pagar`/`fin_caixa_diario` continuam com `CARTAO` genérico |
| `110_church_nucleo_ensino.sql` | `churches.is_nucleo_ensino` (boolean, default false) — flag manual controlado pela secretaria, usado pra filtrar a tela de Matrículas por "igreja núcleo" dentro de um Setor/Regional (pedido do Joaquim, 21/09/2026, achado em teste) |
| `111_rls_escopo_matriculas_financeiro_professores.sql` | **Correção de segurança:** `ead_matriculas`, `fin_contas_receber` e `professores` ainda usavam a policy antiga "é staff (qualquer nível)?", sem checar `unit_id` — diferente de `churches`/`sectors`/`members`/`transactions` (061) e `ead_alunos` (062), que já filtram por `get_accessible_unit_ids()`. Um LOCAL_ADMIN (nível 4, "Acesso ao núcleo de ensino") via/editava matrículas, contas a receber e a lista de professores do sistema inteiro, não só da própria unidade. Também corrige `professores_select_authenticated`, que estava `USING (true)` (qualquer usuário autenticado, nem staff, lia todos os professores). Achado em teste real (Joaquim, 21/09/2026) — ver aviso na resposta sobre o que RLS não cobre (Server Actions com `createAdminClient()`/service_role) |
